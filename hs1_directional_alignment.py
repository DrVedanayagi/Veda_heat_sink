...
def phase1_overview(surface, vessels, vnames, tumors, metrics):
    ...
    # Split viewport: left 25% = info panel | right 75% = 3D scene
    # shape=(1,2) gives two side-by-side sub-plots.
    plotter = pv.Plotter(
    ...
    # RIGHT sub-plot (index 1) -- 3D scene
    plotter.subplot(0, 1)
    plotter.background_color = "black"
    plotter.add_mesh(surface, color="lightgray", opacity=0.07)
    for v, vn in zip(vessels, vnames):
        plotter.add_mesh(v, color=VESSEL_COLOR_MAP.get(vn, "gray"), opacity=0.60)
    ...
    plotter.add_axes(line_width=2, interactive=False)
    plotter.add_text(
        "Drag=rotate  |  Scroll=zoom  |  Shift+drag=pan",
        position="upper_edge", font_size=9, color="gray")

    # LEFT sub-plot (index 0) -- legend / info panel
    plotter.subplot(0, 0)
    plotter.background_color = [0.06, 0.06, 0.08]

    legend_lines = [
        ("PHASE 1 -- Overview", "white"),
        ("Close window to select tumor", "gray"),
        ("", "white"),
        ("-- Structures --", "gray"),
        ("Body Surface", "lightgray"),
    ]
    for vn in vnames:
        col = VESSEL_COLOR_MAP.get(vn, "gray")
        legend_lines.append((vn.replace("_", " ").title(), col))
    legend_lines.append(("", "white"))
    legend_lines.append(("-- Tumors --", "gray"))
    for i, m in enumerate(metrics):
        tc   = TUMOR_COLORS[i % len(TUMOR_COLORS)]
        elig = "ELIGIBLE" if m["eligible"] else "ineligible"
        legend_lines.append(
            (f"Tumor {i+1}  {m['diameter_cm']:.1f} cm  [{elig}]", tc))

    n = len(legend_lines)
    for row, (txt, col) in enumerate(legend_lines):
        if not txt:
            continue
        y_frac = 0.94 - row * (0.90 / max(n, 1))
        plotter.add_text(txt, position=(0.05, y_frac),
                         font_size=11, color=col, viewport=True)
...
def phase3_visualise(surface, vessels, vnames, tumors, centroids,
    ...
    # ---------------------------------------------------------------
    # Split viewport:  left 28% = info/legend panel
    #                  right 72% = 3-D interactive scene
    #
    # Using shape=(1,2) with column_weights so the right panel is
    # wider.  No add_legend() call anywhere -- the left panel renders
    # every label as a coloured text line, completely separate from
    # the 3-D viewport so it never occludes the anatomy.
    # ---------------------------------------------------------------
    plotter = pv.Plotter(
        window_size=[1600, 1000],
        shape="1|2",          # 1 row, 2 cols  (PyVista string shorthand)
        border=False,
        title=(f"Directional MWA v11 -- Tumor {sel_idx+1}  |  "
               f"{power_w:.0f}W x {time_s:.0f}s  |  "
               f"DAS={asi['das_score']:.0f}  |  "
               f"ASI={asi['asi']:.1f} [{asi['risk_label']}]"),
    )

    # ================================================================
    # LEFT sub-plot (0,0) -- static info + legend panel
    # ================================================================
    plotter.subplot(0, 0)
    plotter.background_color = [0.05, 0.05, 0.08]
    ...
    # Title block
    info_sections += [
        (f"Directional MWA  v11", "white"),
        (f"Tumor {sel_idx+1}   {status_tag}", asi_col),
        ("", "white"),
    ]
    ...
    info_sections += [
        ("--- PRESCRIPTION ---", "gray"),
        (f"Power   : {power_w:.0f} W", "white"),
        (f"Time    : {time_s:.0f} s  ({time_s/60:.1f} min)", "white"),
        (f"Fwd zone: {opt_result['zone_diam_fwd_cm']:.2f} cm diam x {opt_result['zone_fwd_cm']:.2f} cm", "white"),
        (f"Rear zone: {opt_result['zone_diam_rear_cm']:.2f} cm diam", "white"),
        ("", "white"),
    ]
    ...
    info_sections += [
        ("--- ASI v11 SCORES ---", "gray"),
        (f"ASI : {asi['asi']:.1f}/100  [{asi['risk_label']}]", asi_col),
        (f"HSS : {asi['hss_score']:.0f}    OCM : {asi['ocm_score']:.0f}", "white"),
        (f"CC  : {asi['cc_score']:.0f}    DRA : {asi['dra_score']:.0f}", "white"),
        (f"DAS : {asi['das_score']:.0f}  (antenna score)", "white"),
        ("", "white"),
    ]
    ...
    info_sections += [
        ("--- OAR STATUS ---", "gray"),
        (f"Rear-protected : {oar_rear}", "lime"),
        (f"Forward risk   : {oar_fwd}", "tomato" if oar_fwd > 0 else "white"),
        ("", "white"),
    ]

    # Structure legend
    info_sections.append(("--- STRUCTURES ---", "gray"))
    info_sections.append(("Body Surface", "lightgray"))
    for vn in vnames:
        is_oar   = any(o["vessel"] == vn for o in oar_list)
        oar_info = next((o for o in oar_list if o["vessel"] == vn), None)
        if is_oar and oar_info and oar_info.get("in_rear_lobe"):
            col  = "lime"
            note = " [rear-protected OAR]"
        elif is_oar:
            col  = "red"
            note = " [forward OAR!]"
        else:
            col  = VESSEL_COLOR_MAP.get(vn, "gray")
            note = ""
        info_sections.append((vn.replace("_", " ").title() + note, col))

    info_sections.append(("", "white"))
    info_sections.append(("--- TUMORS ---", "gray"))
    for i in range(len(tumors)):
        tc    = TUMOR_COLORS[i % len(TUMOR_COLORS)]
        label = f"Tumor {i+1} [TARGET]" if i == sel_idx else f"Tumor {i+1}"
        info_sections.append((label, tc))
    info_sections.append(("  centroid marker", "white"))
    for oar in oar_list:
        if oar["risk"] == "CRITICAL":
            vn  = oar["vessel"]
            col = "lime" if oar.get("in_rear_lobe") else "red"
            info_sections.append((f"OAR exclusion: {vn.replace('_',' ')}", col))

    info_sections.append(("", "white"))
    info_sections += [
        ("--- COLOUR KEY ---", "gray"),
        ("Zone: green=low / yellow / red=high", "lightgray"),
        ("Lime vessel = rear-protected OAR", "lime"),
        ("Red  vessel = forward OAR", "tomato"),
        ("Cyan arrow  = antenna fwd axis", "cyan"),
        ("Magenta     = null (OAR dir)", "magenta"),
        ("Bright ray  = fwd SAR", "white"),
        ("Faint ray   = rear null", "gray"),
        ("", "white"),
        ("--- CONTROLS ---", "gray"),
        ("Left-drag   = rotate", "lightgray"),
        ("Scroll      = zoom", "lightgray"),
        ("Shift+drag  = pan", "lightgray"),
        ("Slider      = time step", "lightgray"),
        ("Play button = animate", "lightgray"),
    ]

    n = len(info_sections)
    for row, (txt, col) in enumerate(info_sections):
        if not txt:
            continue
        y_frac = 0.97 - row * (0.97 / max(n, 1))
        if y_frac < 0.01:
            break
        plotter.add_text(txt, position=(0.04, y_frac),
                         font_size=9, color=col, viewport=True)

    # ================================================================
    # RIGHT sub-plot (0,1) -- full 3-D interactive scene
    # ================================================================
    plotter.subplot(0, 1)
    plotter.background_color = "black"
    ...
    for i, t in enumerate(tumors):
        ...
        plotter.add_mesh(td, color=TUMOR_COLORS[i % len(TUMOR_COLORS)], opacity=op)
    plotter.add_mesh(pv.Sphere(radius=0.006, center=centroid), color="white")
    ...
    def update(t_val):
        ...
        # Animated progress HUD (top-left of the 3-D panel)
        ...
        hud = (
            f"t = {t:.0f} / {time_s:.0f} s    Progress {pct_str}\n"
            f"{zone_str}"
        )
        plotter.add_text(hud, position=(8, 140),
                         font_size=10, color=asi_col, name="hud_anim")
        plotter.render()
    ...
    # Time slider -- bottom strip of the 3-D panel
    ...
    plotter.add_text("Play", position=(52, 40), font_size=10, color="white")
    ...
    plotter.add_text("SAR rays", position=(52, 90), font_size=10, color="yellow")

    # 3-D axes widget (bottom-left of the 3-D panel)
    plotter.add_axes(
        line_width=3,
