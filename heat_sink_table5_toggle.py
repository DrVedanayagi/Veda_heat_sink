#!/usr/bin/env python3
"""
Integrated Heat Sink Analysis — v8
====================================
Changes vs v7:
  COLOR  — Vessel colour scheme corrected:
              aorta        → red  (#FF0000)
              portal_vein  → blue (#1565C0)   (medium blue — distinct from veins)
              hepatic_vein → blue (#1E90FF)   (dodger blue)
              ivc          → blue (#1E90FF)   (same family as hepatic vein)
              hepatic_artery → orange
            OAR-encroached vessels are still highlighted red at full opacity.

  TOGGLE — Ray directional-loss lines now have an ON/OFF checkbox.
            Lines are stored as individual named actors "ray_i_N" so
            they can be removed cleanly when the checkbox is unchecked.
            Default: ON (lines visible on startup).
"""
...
# aorta=red, portal_vein=medium blue, hepatic_vein=dodger blue,
# ivc=dodger blue (same venous family as hepatic vein), hepatic_artery=orange
VESSEL_COLOR_MAP = {
...
def print_re_nu_explanation(per_vessel_hs):
    ...
    print("="*70)
    print(f"  {'Vessel':<18} {'D(mm)':<8} {'u(m/s)':<9} {'Re':<8} {'Regime':<14} {'Nu':<8}")
    print("  " + "-"*70)
    for vn, hs in per_vessel_hs.items():
        print(f"  {vn:<18} {VESSEL_DIAMETERS[vn]*1000:<8.1f} "
              f"{VESSEL_VELOCITIES[vn]:<9.2f} {hs['Re']:<8.0f} "
...
def smooth_tumor(mesh, n_iter=80, relax=0.1):
    try: return mesh.smooth(n_iter=n_iter, relaxation_factor=relax, boundary_smoothing=False)
    except: return mesh
...
def select_regime_oar_safe(tumor_diam_cm, max_loss_pct, centroid_dists_m,
    ...
    def min_wall_clearance(zone_diam_cm):
        zone_r_m = (zone_diam_cm/2.0)/100.0
        return [(vn, centroid_dists_m.get(vn,999.) - VESSEL_RADII.get(vn,0.) - zone_r_m)
                for vn in vnames_present]

    safe_cands = []
    for pw, ts, vol, fwd, diam in ABLATION_TABLE:
        if diam < raw_req: continue
    ...
    if not constrained:
        ...
        cr   = [{"vessel":vn,"wall_clear_mm":cl*1000} for vn,cl in best[6]]
    else:
        ...
        coverage = [(pw,ts,vol,fwd,diam,
                     min(c for _,c in min_wall_clearance(diam)),
                     min_wall_clearance(diam))
                    for pw,ts,vol,fwd,diam in ABLATION_TABLE if diam >= raw_req]
        if not coverage:
            coverage = [(pw,ts,vol,fwd,diam,
                         min(c for _,c in min_wall_clearance(diam)),
                         min_wall_clearance(diam))
...
def compute_staged_plan(centroid, needle_dir, vessels, vnames, centroid_dists, oar_list):
    if oar_list:
    ...
    valid_rows = sorted([r for r in ABLATION_TABLE if r[4] <= max_safe_d_cm],
                        key=lambda r: (-r[4], r[0], r[1]))
    if not valid_rows:
        valid_rows = sorted(ABLATION_TABLE, key=lambda r: r[4])

    sub = valid_rows[0]
    sub_fwd_m  = sub[3] / 100.0
    sub_diam_m = sub[4] / 100.0
...
def run_visualization(surface, vessels, vnames, tumors, centroids,
                      ...
                      particle_systems, constrained, centroid_dists):
    ...
    # Colour rules:
    #   aorta         → red    (arterial)
    #   portal_vein   → medium blue  (portal venous)
    #   hepatic_vein  → dodger blue  (hepatic venous)
    #   ivc           → dodger blue  (systemic venous, same family)
    #   hepatic_artery→ orange
    # If a vessel is an active OAR (encroached by ablation zone at the
    # chosen regime), it is overlaid in bright red at high opacity so
    # it stands out clearly against the blue venous vessels.
    for i, (v, vn) in enumerate(zip(vessels, vnames)):
        is_oar   = any(o["vessel"] == vn for o in oar_list)
        base_col = VESSEL_COLOR_MAP.get(vn, "gray")
        if is_oar:
            # Show in bright red + higher opacity to flag encroachment
            plotter.add_mesh(v, color="red", opacity=0.90,
                             label=f"⚠ OAR: {vn.replace('_',' ').title()}")
        else:
            plotter.add_mesh(v, color=base_col, opacity=0.62,
    ...
    for i, t in enumerate(tumors):
        ...
        label = (f"Tumor {i+1} ablation target" if i == sel_idx else f"Tumor {i+1}")
    ...
    # ── RAY LINES — stored in a list of actor names for toggle ────────
    # Each ray line is added with a unique name "ray_N" so we can
    # selectively remove / re-add them via the toggle checkbox.
    ray_actor_names = []
    ray_meshes      = []   # store (mesh, color, kwargs) for re-adding
    ...
    for oar in oar_list:
        if oar["risk"] == "CRITICAL":
            ...
            centre_pt = oar.get("nearest_pt", centroid)
            excl_sph  = pv.Sphere(radius=excl_r, center=centre_pt,
                                   theta_resolution=20, phi_resolution=20)
            plotter.add_mesh(excl_sph, color="red", opacity=0.18,
                             label=f"OAR exclusion: {vn.replace('_',' ')}")
            plotter.add_mesh(excl_sph, color="red", opacity=0.55,
    ...
    if constrained and oar_list:
        ...
        away_dir    = away_dir / away_norm if away_norm > 1e-6 else np.array([0.,1.,0.])
        arrow_len   = 0.04
        plotter.add_mesh(
            pv.Arrow(start=centroid, direction=away_dir, scale=arrow_len,
        ...
        plotter.add_point_labels(
            ...
            [f"↑ Shift needle this way\nAway from: {vn_oar.replace('_',' ')}\n"
             f"Need: {need_mm:.1f}mm more clearance"],
    ...
    if constrained:
        staged_plan, sub_row = compute_staged_plan(
            centroid, needle_dir, vessels, vnames, centroid_dists, oar_list)
    ...
    def draw_single_zone(frac):
        ...
        if ell is not None:
            plotter.add_mesh(ell, scalars="Temperature_C", cmap="plasma",
                             ...
                             scalar_bar_args={"title":"Temperature (°C)","n_labels":5,
    ...
    def draw_staged_zones(frac):
        ...
        for idx_s, stage in enumerate(staged_plan):
            ell = make_ellipsoid_clean(stage["centre"],
                                       stage["fwd_m"]*frac,
                                       stage["diam_m"]*frac,
                                       needle_dir)
            if ell is not None:
                plotter.add_mesh(ell, scalars="Temperature_C", cmap="plasma",
                                 ...
                                 scalar_bar_args={"title":"Temperature (°C)","n_labels":4,
    ...
    def update(t_val):
        ...
        if mode_state["staged"] and staged_plan:
            ...
            stage_note = (f"  {staged_plan[0]['label']}\n"
                          ...
                          f"  each diam={cur_diam*100:.1f}cm fwd={cur_fwd*100:.1f}cm\n"
                          f"  offset along needle axis\n"
                          f"  30% overlap between zones")
        else:
            cur_fwd, cur_diam = draw_single_zone(frac)
        ...
        if all_pts:
            ...
            plotter.add_mesh(cloud, scalars="blood_velocity_m_s", cmap="coolwarm",
                             ...
                             scalar_bar_args={"title":"Blood velocity (m/s)","n_labels":3,
                                              "label_font_size":10,"title_font_size":11,
        ...
        if constrained:
            worst = min(oar_list, key=lambda o: o["wall_clear_mm"]) if oar_list else None
            warn_line = (f"⚠  CONSTRAINED — vessel encroachment\n"
                         f"  {worst['vessel'].replace('_',' ')} wall: "
                         f"{worst['wall_clear_mm']:.1f}mm [need {OAR_MIN_CLEARANCE_M*1000:.0f}mm]\n"
        ...
        hud = (f"{warn_line}"
               ...
               f"─────────────────────────\n"
               f"Ablation: Purple=37°C White=90°C\n"
               f"Blood:    Blue=slow  Red=fast\n"
               f"─────────────────────────\n"
               f"Vessels:\n"
               f"  Red=Aorta (arterial)\n"
               f"  Blue=Portal vein\n"
               f"  DodgerBlue=Hepatic v+IVC\n"
               f"  Orange=Hepatic artery\n"
               f"Cyan arrow=reposition dir")

        plotter.add_text(hud, position="lower_left", font_size=10,
                         color="tomato" if constrained else "white", name="hud")
        plotter.render()
    ...
    # ── BUTTON COLUMN: Play (row 1), Ray toggle (row 2), Staged (row 3)
    # Each checkbox is at y = 30 + row*70
    ...
    # ── RAY TOGGLE (row 2) ────────────────────────────────────────────
    # The callback removes all ray_N actors when unchecked, and re-adds
    # them (from the saved mesh+color list) when checked again.
    # Default is ON (value=True), matching startup state.
    def toggle_rays(flag):
        mode_state["rays_on"] = bool(flag)
        if flag:
            # Re-add all ray lines
            for name, (line_mesh, col) in zip(ray_actor_names, ray_meshes):
                plotter.add_mesh(line_mesh, color=col, line_width=2.5,
                                 opacity=0.55, name=name)
        else:
            # Remove all ray lines
    ...
    plotter.add_text("◉ Ray lines", position=(80, 108), font_size=11, color="yellow")

    # Row 3 — Staged mode (only when constrained)
    ...
    title_col = "tomato" if constrained else "white"
    plotter.add_text(
...
def main():
    print("="*70)
    print("  HEAT SINK v8 — RAY TOGGLE + CORRECT VESSEL COLOURS")
    print("="*70)

    if not os.path.exists(DATASET_BASE):
        print(f"  ✘ Dataset not found: {DATASET_BASE}"); return
    ...
    for i, path in enumerate(VESSEL_VTK_LIST):
        ...
        if v is not None:
            vessels.append(v); vnames.append(VESSEL_NAMES[i])

    if tumor_mesh is None or surface is None or not vessels:
        print("  ✘ Critical files missing."); return
    ...
    per_vessel_hs = {}
    for vn in vnames:
        per_vessel_hs[vn] = heat_sink_full_physics(centroid_dists[vn], vn, POWER_W, TIME_S)
    ...
    for vn, hs in per_vessel_hs.items():
        ...
        print(f"  {vn:<18}{hs['dist_mm']:<11.1f}{wall_mm:<14.1f}"
              f"{hs['flow_regime']:<14}{hs['Nu']:<8.1f}"
    ...
    print(f"\n  {'⚠  CONSTRAINED' if constrained else '✔  OAR-SAFE'} Regime: "
          f"{rec[0]:.0f}W × {rec[1]:.0f}s  diam={rec[4]:.2f}cm  (need {raw_req:.2f}cm)")
    ...
    oar_list = identify_oars(centroid, vessels, vnames, rec[3], rec[4], safest_dir)
    print(f"\n  OARs encroached: {len(oar_list)}")
    for o in oar_list:
        print(f"    {o['vessel']}  {o['points_inside']} pts  "
              f"centroid={o['closest_mm']:.1f}mm  wall={o['wall_clear_mm']:.1f}mm  [{o['risk']}]")
