#!/usr/bin/env python3
"""
Integrated Heat Sink Analysis — v5 (Color Update + Play Button + Heat Flow)
============================================================================
...
def run_visualization(surface, vessels, vnames, tumors, centroids,
    ...
    # Vessels — OARs highlighted; use updated color map
    VESSEL_COLOR_MAP = {
        "aorta":          "#FF0000",   # red
        "ivc":            "#ADD8E6",   # light blue
        "portal_vein":    "#00008B",   # dark blue
        "hepatic_vein":   "#00008B",   # dark blue
    ...
    # ── TABLE LOOKUP: find the best matching row for a given time ───────
    # Sorted ablation table filtered to the recommended power
    SORTED_TABLE = sorted(ABLATION_TABLE, key=lambda r: (r[0], r[1]))

    def get_table_row(power_w, t_sec):
        """Return (power, time, vol_cc, fwd_cm, diam_cm) closest to power_w and t_sec."""
        candidates = [r for r in SORTED_TABLE if r[0] == round(power_w, 0)]
        if not candidates:
            candidates = SORTED_TABLE
        best = min(candidates, key=lambda r: abs(r[1] - t_sec))
        return best
    ...
    def update(t_val):
        ...
        # ── HUD: table lookup for current time & power ────────────────
    ...
    PLAY_STEP_S  = 5.0        # seconds advanced per timer tick
    TIMER_MS     = 100         # timer interval in ms

    def toggle_play(flag):
        """Called by the play/pause button checkbox."""
    ...
    slider_widget = plotter.add_slider_widget(
        ...
        pointa=(0.22, 0.05), pointb=(0.90, 0.05),   # shifted right to leave room for button
        style="modern")
    ...
    plotter.add_text("▶ Play", position=(80, 38), font_size=11,
                     color="white")
...
def main():
    print("=" * 70)
    print("  HEAT SINK v5 — COLORS + PLAY BUTTON + HEAT ARROWS + RAY LINES")
    print("=" * 70)
