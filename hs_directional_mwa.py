...
def create_directional_heat_arrows(centroid, vessels, vnames,
    ...
    """
    Visualise heat flow arrows with directional SAR information.
    Arrow colour still encodes loss_pct.
    Arrow opacity now also reflects SAR weight (protected vessels = faint).
    A thick cyan arrow shows the antenna forward axis.
    A thin dashed magenta arrow shows the null direction (toward OAR).
    """
    ...
    if ax_n > 1e-6:
        ...
        plotter.add_point_labels(
            pv.PolyData([centroid + ax_unit * 0.07]),
            ["→ Antenna axis (SAR max)"],
        ...
        plotter.add_point_labels(
            pv.PolyData([centroid - ax_unit * 0.05]),
            ["◉ Null (OAR protected)"],
            font_size=9, text_color="magenta",
            point_size=1, always_visible=True, shape_opacity=0.0)

    plotter.add_text(
        "Heat Flow: Green=Low  Yellow=Mid  Red=High\\n"
        "Opacity=SAR weight  Cyan=Fwd axis  Magenta=Null",
...
def phase3_visualise(surface, vessels, vnames, tumors, centroids,
    ...
    def clear_dynamic():
        for nm in ["ablation_fwd", "ablation_rear", "particles", "hud"]:
            try: plotter.remove_actor(nm)
            except: pass

    def draw_zone(frac):
        """Draw D-shaped zone as two half-ellipsoids."""
        zone = make_dshaped_zone(centroid, fwd_m, diam_m, antenna_ax, frac=frac)
        if zone is not None and zone.n_points > 0:
            try:
                plotter.add_mesh(zone, scalars="Temperature_C", cmap="plasma",
    ...
    def update(t_val):
        ...
        zone_str = (f"Zone fwd: {cur_fwd*100:.1f}cm  rear: {opt_result['zone_diam_rear_cm']:.1f}cm"
                    if frac > 0.01 else "Zone: growing...")
        oar_rear = sum(1 for o in oar_list if o.get("in_rear_lobe"))
        oar_fwd  = len(oar_list) - oar_rear
        warn = ("⚠  CONSTRAINED — reposition\\n" if constrained else "✔  OAR-SAFE\\n")

        hud = (
            f"{'─'*34}\\n"
            f"  ASI v11 — Directional MWA\\n"
            f"  ASI={asi['asi']:.1f}/100  [{asi['risk_label']}]\\n"
            f"  HSS={asi['hss_score']:.0f} OCM={asi['ocm_score']:.0f} "
            f"CC={asi['cc_score']:.0f}\\n"
            f"  DRA={asi['dra_score']:.0f} DAS={asi['das_score']:.0f}(NEW)\\n"
            f"{'─'*34}\\n"
            f"  Directional Antenna v11\\n"
            f"  G_fwd ={G_FORWARD:.2f}× G_rear={G_REAR:.2f}×\\n"
            f"  P={power_w:.0f}W  t={time_s:.0f}s ({time_s/60:.1f}min)\\n"
            f"  Q_sink={opt_result['Q_sink_W']:.2f}W (dir.weighted)\\n"
            f"  Dose ×{opt_result['dose_sf']:.2f}\\n"
            f"{'─'*34}\\n"
            f"  Zone (D-shaped)\\n"
            f"  Fwd:{opt_result['zone_diam_fwd_cm']:.1f}cm "
            f"Rear:{opt_result['zone_diam_rear_cm']:.1f}cm\\n"
            f"{'─'*34}\\n"
            f"{warn}"
            f"  t={t:.0f}s/{time_s:.0f}s ({frac*100:.0f}%)\\n"
            f"  {zone_str}\\n"
            f"{'─'*34}\\n"
            f"  OARs: {len(oar_list)} total\\n"
            f"  Rear-protected: {oar_rear} (lime=safe)\\n"
            f"  Forward risk:   {oar_fwd} (red=check)\\n"
            f"{'─'*34}\\n"
            f"  Cyan = antenna forward axis\\n"
            f"  Magenta = null (OAR protected)\\n"
        )
    ...
    def toggle_rays(flag):
        ...
        if flag:
            for nm, (lm, col) in zip(ray_actor_names, ray_meshes_list):
    ...
    plotter.add_text("◉ SAR rays", position=(80, 108),
                     font_size=11, color="yellow")

    plotter.add_legend(loc="upper right", size=(0.30, 0.52))
    plotter.add_text(
        f"Directional MWA v11  |  {power_w:.0f}W × {time_s:.0f}s  |  "
        f"G_fwd={G_FORWARD:.1f}× G_rear={G_REAR:.1f}×  |  "
